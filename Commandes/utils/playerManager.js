...
module.exports.play = (client, msg) => {
    try {
        ...
        if (!player) return msg.channel.send("❌ Je ne suis pas connecter a un salon vocal !");
        ...
        player.once("end", (data) => {
            if ((data.reason === "REPLACED") || (data.reason === "Stoper" && queue.length === 0)) return;
    ...
    try {
        ...
        if (type.name === "ytsearch" || type.name === "scsearch") {
            ...
            if (songs.length > 1 && !type.now) {
                ...
                model.then(async (m) => {
                    ...
                    collector.on("collect", async (msgCollected) => {
                        ...
                        if (choice.toLowerCase() === "cancel") {
                        	...
                        	return msg.channel.send("✅ J'ai réussi a me connecter !");
                        }

                        let choices = ["1", "2", "3", "4", "5", "cancel"];
                        if (isNaN(choice)) return;
                        if (!choices.includes(choice)) return msg.channel.send("❌ Le channel est introuvable !");
                ...
                const song = songs[0];
                if (!song) return msg.channel.send("⚠ Aucune misique trouvée !");
...
module.exports.addRadio = async(client, msg, track) => {
    try {
        ...
        const songs = await this.getSongs(client.manager, track);
        if (!songs) return msg.channel.send("⚠ Aucune radio trouvée !");
